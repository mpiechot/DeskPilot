...
const available = resolveLatestReleaseUpdate("1.1.0", {
  tag_name: "v1.1.1",
  html_url: "https://github.com/mpiechot/DeskPilot/releases/tag/v1.1.1",
  draft: false,
  prerelease: false
});
assert(available.status === "available", "Expected a newer stable GitHub release to be available");
assert(available.availableVersion === "1.1.1", "Expected the normalized available version");

const invalidUrl = resolveLatestReleaseUpdate("1.1.0", {
  tag_name: "v1.1.1",
  html_url: "https://example.com/deskpilot/releases/v1.1.1",
  draft: false,
  prerelease: false
});
assert(invalidUrl.status === "unavailable", "Expected a non-GitHub release URL to fail closed");

const prerelease = resolveLatestReleaseUpdate("1.1.0", {
  tag_name: "v1.2.0-beta.1",
  html_url: "https://github.com/mpiechot/DeskPilot/releases/tag/v1.2.0-beta.1",
  draft: false,
  prerelease: true
});
